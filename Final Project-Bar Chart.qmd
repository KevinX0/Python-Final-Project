...
suburban_damage_counts = suburban_damage_counts.

```

# pie chart for destory
```{python}
# Filter data for 2020-2023 and "Destroyed (>50%)" damage level
destroyed_data = merged_data[
    (merged_data["Year"] >= 2020) & 
    (merged_data["Year"] <= 2023) &
    (~merged_data["geographic type"].isna()) &
    (merged_data["* Damage"] == "Destroyed (>50%)")
].copy()

# Group data by geographic type and calculate counts for "Destroyed" damage level
destroyed_counts = (
    destroyed_data.groupby(["geographic type"])
    .size()
    .reset_index(name="Count")
)
